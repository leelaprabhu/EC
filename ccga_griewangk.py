...
def griewangk(x):
    ...
    for i in range(0,len(x)):
        ...
        pi1= pi1*math.cos(x[i]/math.sqrt(i+1))
    f_x=1.0 + sig - pi1
    return f_x
...
def findFit(i,indi,bestIndi,lower,upper,prec):
    ...
    for j in range(0,len(indi)):
        ...
        fit2.append(griewangk(process(temp,lower,upper,prec)))
    fitMax= max(fit2)*1.05
    fit3=[]
    for j in range(0, len(indi)):
        fit3.append(1000-fit2[j])
    return fit3
...
while(countFunc<100000):
    ...
    fprev.append(fmin)
    for i in range(0,noVar):
        results.append(griewangk(process(bestIndi,lower,upper,prec)))
        counts.append(countFunc)
        print str(griewangk(process(bestIndi,lower,upper,prec)))+" "+str(countFunc)
