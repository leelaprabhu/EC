...
def schwefel(x):
    ...
    for i in range(0,len(x)):
        f_x=f_x + (x[i]*math.sin(math.sqrt(abs(x[i]))))
    return f_x
...
def findFit(pop, lower, upper, prec):
    ...
    for i in range(0, len(pop)):
        ...
        fit2.append(schwefel(process(pop[i],lower,upper,prec)))
...
while(countFunc<100000):
    results.append(schwefel(process(pop[fit.index(max(fit))],lower,upper,prec)))
    counts.append(countFunc)
    print str(schwefel(process(pop[fit.index(max(fit))],lower,upper,prec)))+" "+str(countFunc)
    gen=gen+1
