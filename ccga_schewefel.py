...
def findFit(i,indi,bestIndi,lower,upper,prec):
    ...
    for j in range(0,len(indi)):
        ...
        fit2.append(schwefel(process(temp,lower,upper,prec)))
    fitMax= max(fit2)*1.05
    fit3=[]
    for j in range(0, len(indi)):
        fit3.append(500-fit2[j])
    return fit3
...
while(countFunc<100000):
    ...
    fprev.append(fmin)
    for i in range(0,noVar):
        results.append(schwefel(process(bestIndi,lower,upper,prec)))
        counts.append(countFunc)
        print str(schwefel(process(bestIndi,lower,upper,prec)))+" "+str(countFunc)
