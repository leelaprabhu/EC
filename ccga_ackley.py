...
def findFit(i,indi,bestIndi,lower,upper,prec):
    ...
    for j in range(0,len(indi)):
        ...
        fit2.append(ackley(process(temp,lower,upper,prec)))
...
while(countFunc<100000):
    ...
    fprev.append(fmin)
    for i in range(0,noVar):
        results.append(ackley(process(bestIndi,lower,upper,prec)))
        counts.append(countFunc)
        print str(ackley(process(bestIndi,lower,upper,prec)))+" "+str(countFunc)
