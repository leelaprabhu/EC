...
def griewangk(x):
    ...
    for i in range(0,len(x)):
        sig= sig+(x[i]*x[i]/4000.0)
        pi= pi*math.cos(x[i]/math.sqrt(i+1))
    f_x=1.0 + sig - pi
    return f_x
...
def findFit(pop, lower, upper, prec):
    ...
    for i in range(0, len(pop)):
        ...
        fit2.append(griewangk(process(pop[i],lower,upper,prec)))
...
while(countFunc<100000):
    results.append(griewangk(process(pop[fit.index(max(fit))],lower,upper,prec)))
    counts.append(countFunc)
    print str(griewangk(process(pop[fit.index(max(fit))],lower,upper,prec)))+" "+str(countFunc)
    gen=gen+1
